...
export const Footer = ({
  ...
  return (
    <footer
      ref={footerRef}
      className={cn('relative w-full border-t border-gray-800 bg-black', className)}
    >
      <div className="relative w-full px-4 py-8 sm:px-6 sm:py-10 md:py-12">
        ...
        <div className="flex min-h-[22rem] flex-col justify-between sm:min-h-[26rem] md:min-h-[30rem]">
          <div className="mb-10 flex w-full flex-col sm:mb-14 md:mb-16">
            <div className="flex w-full flex-col items-center">
              ...
              >
                <p className="w-full max-w-xs px-2 text-center text-sm leading-relaxed font-normal text-gray-300 sm:max-w-sm sm:text-base md:max-w-xl md:text-lg">
                  {brandDescription}
                </p>
              </div>
                ...
                >
                  {socialLinks.map((link, index) => (
                    ...
                    >
                      ...
                      <div className="relative flex h-9 w-9 transform items-center justify-center rounded-full border-2 border-gray-700 bg-gray-900 transition-all duration-300 group-hover:-translate-y-1 group-hover:scale-110 group-hover:border-blue-400 sm:h-10 sm:w-10 md:h-12 md:w-12">
                        {/* Glow effect behind */}
                        ...
                        <div className="h-4 w-4 text-gray-400 transition-colors duration-300 group-hover:text-white sm:h-5 sm:w-5 md:h-6 md:w-6">
                          {link.icon}
                        </div>
                      </div>
              ...
              {navLinks.length > 0 && (
                <div
                  ref={navRef}
                  className="flex flex-wrap justify-center gap-4 text-xs font-medium text-gray-400 sm:gap-6 sm:text-sm md:gap-8 md:text-base"
                >
          ...
          <div className="mt-12 flex flex-col items-center justify-center gap-2 text-xs sm:mt-16 sm:text-sm md:mt-20 md:flex-row md:items-center md:justify-between">
            ...
            {creatorName && creatorUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="cursor-pointer text-gray-500 transition-all duration-300 hover:font-medium hover:text-white"
                style={{ zIndex: 100, pointerEvents: 'auto' }}
              >
        ...
        <div
          ref={logoRef}
          className="absolute bottom-16 left-1/2 -translate-x-1/2 cursor-pointer rounded-2xl border-2 border-gray-800 bg-black/90 p-2 shadow-2xl backdrop-blur-md transition-all duration-500 hover:border-blue-400 hover:shadow-blue-500/40 sm:bottom-18 sm:rounded-3xl sm:p-3 md:bottom-20"
          style={{ zIndex: 100, pointerEvents: 'auto' }}
        >
          {/* Glow behind */}
        ...
        {/* Bottom line */}
        <div
          className="pointer-events-none absolute right-0 bottom-24 left-0 h-px bg-gradient-to-r from-transparent via-blue-500/20 to-transparent sm:bottom-26 md:bottom-28"
          style={{ zIndex: 1 }}
        ></div>

        {/* Bottom shadow */}
        <div
          className="pointer-events-none absolute right-0 bottom-20 left-0 h-16 bg-gradient-to-t from-black via-black/90 to-black/40 sm:bottom-22 sm:h-20 md:bottom-24"
          style={{ zIndex: 1 }}
        ></div>
      </div>
    </footer>
